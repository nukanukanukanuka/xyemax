...
OWN_VIEWER_IDS = {acc["viewer_id"] for acc in ACCOUNTS}
...
class MaxTransport:
    def __init__(self, role: str, token: str, viewer_id: int,
        ...
        self._BATCH_DELAY = 0.0   # без задержки — батчинг происходит естественно под нагрузкой
    ...
    async def send(self, obj: dict):
        ...
        if len(self._send_buffer) >= self._BATCH_MAX:
            ...
            if not self._send_queue.empty():
                # Очередь занята — ждём, чтобы накопить батч
                delay = self._BATCH_DELAY
            elif total_buffered < 512:
                # Маленький/контрольный фрейм — ждём 25мс, вдруг следом идёт data
                delay = 0.025
            else:
                # Есть реальные данные — отправляем немедленно
                delay = 0.0
    ...
    async def _send_file(self, file_body: bytes):
        """Загрузить файл через MAX File API и опубликовать attachment строго после op136."""
        http = self._http
        ...
        # 2. Уведомление о загрузке файла (opcode 65) и ожидание op136
        ...
        # 4. Дождаться op136 — только после этого публиковать сообщение с attach
        try:
            ...
            self._once.pop(f"op136_{file_id}", None)
            # Файл уже загружен — отправляем op64 в любом случае
    ...
    async def _recv_loop(self):
        async for raw in self.ws:
            ...
            # В self-chat сообщение с вложением/текстом может прийти как ack op64/cmd1,
            # а не только как event op128/cmd0.
            if (op == 128 and cmd == 0) or (op == 64 and cmd == 1 and isinstance(pl, dict) and "message" in pl):
                ...
                log.debug(f"[transport] msg-event op={op} cmd={cmd} chat={chat_id} sender={sender} attaches={len(attaches)}")
                ...
                asyncio.create_task(self._recv_file(file_id, str(msg_id), chat_id, sender))
    ...
    async def _resolve_download_url(self, file_id: int, msg_id: str, chat_id: int) -> str | None:
        ...
        При таймауте делает до 3 попыток с нарастающим ожиданием.
        """
    ...
    async def _recv_file(self, file_id: int, msg_id: str, chat_id: int, sender):
        ...
        for obj in frames:
            ...
            log.debug(f"[transport:{self.label}] frame  action={obj.get('a')}  id={obj.get('id')}  size={len(obj.get('d',''))}")
            if self.on_frame:
...
class ProxyServer:
    ...
    async def _try_finalize_close(self, cid):
        if cid not in self._closing or cid not in self.conns:
            return
        last_seq = self._closing[cid]
        next_seq = self._next_seq.get(cid, 0)
        buf = self._buf.get(cid, {})
        if next_seq > last_seq and not buf:
            log.info(f"[{cid}] finalize close (all data received, last_seq={last_seq})")
    ...
    async def _do_close(self, cid, notify):
        ...
        for d in (self._stats, self._seq, self._next_seq, self._buf, self._closing): d.pop(cid, None)
...
class SessionManager:
    ...
    async def start(self):
        ...
        if migrated:
            ...
            log.info(f"[sessions] мигрированы в self-chat режим ({SESSIONS_FILE})")
    ...
    async def _run_proxy(self, url: str, chat_id: int, name: str):
        ...
        while True:
            transports = [
                MaxTransport("server", acc["token"], acc["viewer_id"], acc["device_id"], chat_id, label=acc["label"])
                for acc in ACCOUNTS
            ]
...
async def main():
    ...
    _console("=" * 50)
    log.info(f"MAX Proxy SERVER запущен  viewer_id={VIEWER_ID}  api={STATUS_URL}  poll={POLL_INTERVAL}s  transports={len(ACCOUNTS)}")
